...
class PaperBroker:
    ...
    def update_dynamic_atr_exits(
        ...
        *,
        ...
        # Mutual exclusivity: If ATR trailing is enabled, fixed percent trailing is ignored for this position.
        # This is implicitly handled because update_dynamic_atr_exits and _update_trailing_stop 
        # both update position.stop_loss, but ATR exits run after the fixed trail update in process_candle.
        # Actually, let's explicitly flag it to prevent fixed trail from fighting.
        if trailing_enabled:
            position.metadata["fixed_trailing_stop_suppressed"] = True
    ...
    def _update_trailing_stop(
        ...
        if not self.trailing_stop_enabled:
            return
        # Mutual exclusivity: Suppress fixed trail if ATR trailing has marked it as suppressed.
        if position.metadata.get("fixed_trailing_stop_suppressed"):
            return
