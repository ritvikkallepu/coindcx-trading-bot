...
class HybridMetaStrategy(Strategy):
    ...
    exit_threshold: Decimal = Decimal("0.45")
    min_volume_ratio: Decimal = Decimal("0.65")
    ...
    allow_short_without_open_interest: bool = False
    ...
    def evaluate(self, context: StrategyContext) -> StrategySignal:
        ...
        raw_direction = _raw_candidate_direction(final_score, quality)
        ...
        if visual["blocked"]:
            return self._hold(context, f"Visual screen blocked: {visual['reason']}", metadata)
        ...
        if quality["mode"] == "tiered":
            ...
            if final_score <= -quality["short_entry_threshold"]:
                short_block = _short_block_reason(
                    final_score=final_score,
                ...
                if short_block is not None:
                    return self._hold(context, short_block, metadata)
                decision = _quality_decision(
                    direction=SignalDirection.SHORT,
                    final_score=final_score,
                    agreement_ratio=short_agreement,
            ...
            return self._hold(context, "Hybrid score below tiered entry threshold.", metadata)
        ...
        return self._hold(context, "Hybrid score below entry threshold.", metadata)
    ...
    def _ema_rsi_score(
        ...
        *,
        ...
        spread_pct = ((fast_current - slow_current) / latest_close) * Decimal("100")
        ...
        *,
        ...
        if volume_ratio < self.min_volume_ratio:
            return {
                "score": Decimal("0"),
                "blocked": True,
                "reason": f"volume ratio too low: {volume_ratio}",
        ...
        score = _clamp((momentum_score * Decimal("0.7")) + (structure_score * Decimal("0.3")), Decimal("-1"), Decimal("1"))
    ...
    def _combined_score(
        ...
        *,
        ...
        active_weight = self.ema_weight + self.bb_weight + self.visual_weight
        if oi_active:
...
def _raw_candidate_direction(
    ...
    long_threshold = quality["long_entry_threshold"]
    short_threshold = quality["short_entry_threshold"]
    if final_score >= long_threshold:
        return SignalDirection.LONG
    if final_score <= -short_threshold:
        return SignalDirection.SHORT
...
def _short_block_reason(
    *,
    ...
    if agreement_ratio < quality["short_agreement_threshold"]:
        return "Short setup blocked: component agreement is below the short threshold."
    if oi_metadata.get("score_used") and oi_score > 0:
        return "Short setup blocked: open interest contradicts the short."
    if oi_metadata.get("score_used"):
        return None
    if allow_short_without_open_interest:
        return None
    if not controlled_shorts:
        return "Short setup requires open-interest confirmation."
    if abs(final_score) < quality["short_entry_threshold"]:
        return "Short setup blocked: no-OI short needs stronger score confirmation."
    return None
...
class HybridMetaV2Strategy(HybridMetaStrategy):
    ...
    exit_threshold: Decimal = Decimal("0.35")
    min_volume_ratio: Decimal = Decimal("0.75")
